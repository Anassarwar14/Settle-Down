...
const CreateListing = () => {
  ...
  return (
    <main className='sm:py-5 sm:px-10 mx-auto'>
      <form className='grid grid-cols-1 sm:grid-cols-5 sm:gap-6 '>
        <section className='flex flex-col gap-4 col-span-1 sm:col-span-3 bg-gray-100 border rounded-md p-4 '>
          <div className="flex items-center justify-between">
            ...
            <button type='submit' className='px-3 pr-4 py-3 bg-teal-500 rounded-xl shadow-md enabled:hover:shadow-cyan-300 text-white flex items-center gap-2 font-light disabled:bg-gray-300 disabled:cursor-not-allowed'>
          ...
          <article className='grid grid-cols-2 gap-2 gap-x-4'>
            ...
            <div>
              <div className="flex items-center gap-2">
                <input onChange={handleChange} id='parking' type='checkbox' className='text-xs accent-teal-600 rounded'/>
                <label htmlFor='parking' className='text-sm'>Parking</label>
              </div>
              <div className="flex items-center gap-2">
                <input onChange={handleChange} id='furnished' type='checkbox' className='text-xs accent-teal-600 rounded'/>
          ...
          <article onChange={(e) => {toggleRent(); handleChange(e);}} >
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="sell" className='text-sm'>Sell</label>
            </div>
          </article>
          <div className="flex items-center gap-2">
            <input onChange={ (e) => {handleChange(e); setIsOffer(!isOffer);}} id='offer' type='checkbox' className='text-xs accent-teal-600 rounded'/>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center justify-around bg-gray-50 border rounded-md p-2 shadow-sm text-zinc-700">
              <input required onChange={handleChange} id='regularPrice' type='number' className='bg-gray-50 border rounded-md w-4/5 focus:outline-none'/>
            ...
            <div className={`flex items-center justify-around bg-gray-50 border rounded-md p-2 shadow-sm text-zinc-700 ${!isOffer && 'hidden'}`}>
              <input required onChange={handleChange} id='discountPrice' type='number' className='bg-gray-50 border rounded-md w-4/5 focus:outline-none'/>
              <BsCurrencyDollar className='text-teal-500'/> {isRent && <small>/ month</small>}
            </div>
          </div>
        </section>
        ...
        <aside className='h-max bg-gray-100 border rounded-md p-5 col-span-2'>
          <section className='flex flex-col gap-6'>
                  ...
                  {formData.imageURLs.map((url, index) => 
                    (
                      <div key={url} className='w-44 flex items-center justify-between text-xs text-zinc-700'>
                        ...
                        {fileNames[index] && <h4>{fileNames[index]}</h4>}
            ...
            <textarea name="description" id="description" placeholder='Highlight the main selling points...' className='caret-teal-500 focus:outline-teal-500 min-h-36 border shadow-sm rounded-lg bg-gray-50 p-3 text-zinc-700'></textarea>
          </section>
        </aside>
      </form>
    </main>
