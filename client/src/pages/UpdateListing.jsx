...
import { MdError, MdErrorOutline, MdUpdate, MdUpload } from 'react-icons/md';
...
const UpdateListing = () => {
    ...
    function handleImageRemove (index) {
      setFormData((prev) => {
        return {...prev, imageURLs: formData.imageURLs.filter((_, i) => i !== index)}
      })
        ...
        <form onSubmit={handleSubmit} className='grid grid-cols-1 sm:grid-cols-5 sm:gap-6 '>
          <section className='flex flex-col gap-4 col-span-1 sm:col-span-3 bg-gray-100 border rounded-md p-4 '>
            <div className="flex items-center justify-between">
              ...
              <button disabled={error || loading || uploading || noChange} type='submit' className={`p-3 w-44 enabled:bg-teal-500 rounded-xl shadow-md enabled:hover:ring-4 enabled:hover:ring-cyan-200 transition-all ease-in-out duration-300 text-white flex items-center justify-center gap-3 font-light ${error ? 'disabled:bg-red-500' :  'disabled:bg-gray-300'} disabled:cursor-not-allowed`}>
                {(loading) ? <><BsHouseUp className='text-xl animate-pulse' /> Updating...</> : (!error && !success) ? <><BsHouseAdd className='text-xl'/> Update Property</> : 
                error ? <><BsHouseExclamation className='text-xl' /> Update Error</> :
                success && <><BsHouseCheck className='text-xl' /> Updated</>}
              </button>
            </div>
            ...
            <input required onChange={handleChange} id="name" value={formData.name} maxLength={100} className='w-full text-zinc-700 rounded-md border shadow-sm p-2 bg-gray-50 focus:outline-none' />
            ...
            <input required value={formData.address} onChange={handleChange} id='address' type='address'  className='w-full rounded-md border shadow-sm p-2 bg-gray-50 focus:outline-none text-zinc-700' />
            ...
            <article className='grid grid-cols-2 gap-2 gap-x-4'>
              ...
              <select value={formData.bedrooms} onChange={handleChange} id='bedrooms' className='bg-gray-50 border rounded-md shadow-sm p-2 text-zinc-700'>
              ...
              <select value={formData.bathrooms} onChange={handleChange} id='bathrooms' className='bg-gray-50 border rounded-md shadow-sm p-2 text-zinc-700'>              
                <option value={1} >1 Room</option>
                <option value={2} >2 Room</option>
                <option value={3} >3 Room</option>
                <option value={4} >4 Room</option>
              ...
              <div>
                <div className="flex items-center gap-2">
                  <input value={formData.parking} checked={formData.parking} onChange={handleChange} id='parking' type='checkbox' className='text-xs accent-teal-600 rounded'/>
                  <label htmlFor='parking' className='text-sm'>Parking</label>
                </div>
                <div className="flex items-center gap-2">
                  <input value={formData.furnished} checked={formData.furnished} onChange={handleChange} id='furnished' type='checkbox' className='text-xs accent-teal-600 rounded'/>
            ...
            <article onChange={handleChange}>
              <div className="flex items-center gap-2">
                <input required id='type' checked={formData.type === 'rent'} value="rent" name='radio' type='radio' className='text-xs accent-teal-600 rounded '/>
                <label htmlFor='type' className='text-sm'>Rent</label>
              </div>
              <div className="flex items-center gap-2">
                <input required checked={formData.type === 'sell'} value="sell" id='type' name='radio' type='radio' className='text-xs accent-teal-600 rounded'/>
                <label htmlFor='type' className='text-sm'>Sell</label>
              </div>
            </article>
            <div className="flex items-center gap-2">
              <input checked={formData.offer} onChange={handleChange} id='offer' type='checkbox' className='text-xs accent-teal-600 rounded'/>
              <label htmlFor="offer" className='text-sm'>Discount Offer</label>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <label htmlFor="regularPrice" className={`${!formData.offer && 'col-span-2'}`}>{formData.type === 'rent' ? "Monthly Payment" : "Regular Price"}</label>
              <label hidden={!formData.offer} htmlFor="discountPrice">Discounted Price</label>
              <div className="flex items-center justify-around bg-gray-50 border rounded-md p-2 shadow-sm text-zinc-700">
                ...
                <BsCurrencyDollar className='text-teal-500'/>{formData.type === 'rent' && <small>/ month</small>}
              </div>
              <div className={`flex items-center justify-around bg-gray-50 border rounded-md p-2 shadow-sm text-zinc-700 ${!formData.offer && 'hidden'}`}>
                <input required={formData.offer} value={formData.discountPrice} onChange={handleChange} id='discountPrice' type='number' min={0} max={100000000000-1} className='bg-gray-50 border rounded-md w-4/5 focus:outline-none'/>
                <BsCurrencyDollar className='text-teal-500'/> {formData.type === 'rent' && <small>/ month</small>}
              </div>
            </div>
          </section>
