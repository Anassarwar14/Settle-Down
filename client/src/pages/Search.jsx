...
const Search = () => {
    ...
    if (
      ...
      offerTermFromUrl
        ...
        setFilterData((prev) => ({
          ...
          city: cityTermFromUrl || prev.city,
          bedrooms: bedroomsTermFromUrl || prev.bedrooms,
          bathrooms: bathroomsTermFromUrl || prev.bathrooms,
          price: priceTermFromUrl || prev.price,
          type: typeTermFromUrl || prev.type,    
          furnished: furnishedTermFromUrl === 'true' ? true : prev.furnished,
          parking: parkingTermFromUrl === 'true' ? true : prev.parking,
          offer: offerTermFromUrl === 'true' ? true : prev.offer,
        }))
      }
  ...
  function updateSearchQuery () {
      ...
      for (const key in filterData) {
        if(key === 'priceHightoLow') continue;
        if (filterData[key] !== '') urlParams.set(key, filterData[key]);
      }
    ...
    <main>
      <header className='max-sm:flex max-sm:flex-col-reverse relative'>
        <section className='mt-1 sm:mt-7 flex max-w-[76rem] ml-2 sm:mx-auto gap-20'>
          ...
          <div className='flex flex-wrap gap-12 items-center *:text-sm max-sm:hidden '>
              <Select  
                id='country'
        ...
        <section className='mt-2 sm:mt-5 flex sm:mx-auto max-w-[78rem] items-center justify-between sm:border max-sm:border-y max-sm:border-teal-400 px-2 sm:pl-2 sm:pr-5 py-3 sm:py-1  sm:rounded-full bg-slate-50 '>
          ...
          <div className='flex sm:gap-[10rem] mr-2 sm:mr-8 max-sm:gap-4'>
              <button type='button' className='flex max-sm:flex-row-reverse max-sm:font-light items-center gap-2 text-stone-700 hover:bg-cyan-50 transition ease-in-out duration-200' onClick={() => setFilterData((prev) => ({...prev, priceHightoLow: !prev.priceHightoLow, order: !prev.priceHightoLow ? 'desc' : 'asc'}))}>
                ...
                <p className='text-sm max-sm:text-teal-700'>Price</p>
              </button>
        ...
        >
          <form onSubmit={handleExtraSubmit} className="p-4 flex flex-col gap-[30vh] h-screen">
            <div className="mt-4">
            ...
            <div className='flex flex-col gap-4'>
              ...
              <button type='submit' className=' transition ease-in duration-150 bg-teal-500 text-white rounded-full px-6 py-2 shadow-sm'>
                Save 
              </button>
              <button
                type='button'
